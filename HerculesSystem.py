...
def export_data():
    ...
    try:
        ...
        for setting in PayrollSettings.query.all():
            data['payroll_settings'].append({
                'id': setting.id,
        ...
        # Export Payroll Components
        for component in PayrollComponent.query.all():
            data['payroll_components'].append({
                'id': component.id,
                'name': component.name,
        ...
        # Export Employee Payroll Adjustments
        for adjustment in EmployeePayrollAdjustment.query.all():
            data['employee_payroll_adjustments'].append({
                'id': adjustment.id,
                'employee_id': adjustment.employee_id,
                'pay_period': adjustment.pay_period,
                'adjustment_type': adjustment.adjustment_type,
                'amount': float(adjustment.amount) if adjustment.amount else 0,
        ...
        # Export Payroll Audit Trail
        for audit in PayrollAuditTrail.query.all():
            data['payroll_audit_trail'].append({
                'id': audit.id,
                'employee_id': audit.employee_id,
                'pay_period': audit.pay_period,
                'action': audit.action,
        ...
        for leave in LeaveRequest.query.all():
        ...
        for balance in LeaveBalance.query.all():
        ...
        # Export Leave Balance History
        for history in LeaveBalanceHistory.query.all():
            data['leave_balance_history'].append({
                'id': history.id,
                'employee_id': history.employee_id,
                'admin_id': history.admin_id,
                'leave_type': history.leave_type,
                'old_total': history.old_total,
                'new_total': history.new_total,
                'old_used': history.old_used,
                'new_used': history.new_used,
                'old_remaining': history.old_remaining,
                'new_remaining': history.new_remaining,
        ...
        return Response(
            ...
            headers={
                "Content-Disposition": "attachment; filename=hercules_backup.json",
                "Content-Type": "application/json"
            }
        )
...
def import_data():
    ...
    try:
        ...
        # Create ID mapping to handle conflicts
        employee_id_mapping = {}

        # Disable autoflush to prevent premature commits
        with db.session.no_autoflush:

            # STEP 1: Import Employees (base table) - let database assign new IDs
            ...
            for balance_data in data.get('leave_balances', []):
                ...
                if not existing:
                    new_employee_id = employee_id_mapping.get(balance_data['employee_id'])
                    if new_employee_id:
                        balance = LeaveBalance(
                            id=balance_data['id'],
                            employee_id=new_employee_id,
            ...
            # STEP 5: Import Employee Payroll Adjustments (depends on employees) - use ID mapping
            ...
            # STEP 6: Import Payroll Audit Trail (depends on employees) - use ID mapping
            print("Importing payroll audit trail...")
            for audit_data in data.get('payroll_audit_trail', []):
                existing = PayrollAuditTrail.query.filter_by(id=audit_data['id']).first()
                if not existing:
                    new_employee_id = employee_id_mapping.get(audit_data['employee_id'])
                    new_performed_by = employee_id_mapping.get(audit_data['performed_by'])
                    if new_employee_id and new_performed_by:
                        audit = PayrollAuditTrail(
                            id=audit_data['id'],
                            employee_id=new_employee_id,
            ...
            # STEP 7: Import Leave Requests (depends on employees) - use ID mapping
            print("Importing leave requests...")
            for leave_data in data.get('leave_requests', []):
                existing = LeaveRequest.query.filter_by(id=leave_data['id']).first()
                if not existing:
                    new_employee_id = employee_id_mapping.get(leave_data['employee_id'])
                    new_approved_by = employee_id_mapping.get(leave_data['approved_by_id']) if leave_data['approved_by_id'] else None
                    if new_employee_id:
                        leave = LeaveRequest(
                            id=leave_data['id'],
                            employee_id=new_employee_id,
            ...
            # STEP 8: Import Leave Balance History (depends on employees) - use ID mapping
            print("Importing leave balance history...")
            for history_data in data.get('leave_balance_history', []):
                existing = LeaveBalanceHistory.query.filter_by(id=history_data['id']).first()
                if not existing:
                    new_employee_id = employee_id_mapping.get(history_data['employee_id'])
                    new_admin_id = employee_id_mapping.get(history_data['admin_id'])
                    if new_employee_id and new_admin_id:
                        history = LeaveBalanceHistory(
                            id=history_data['id'],
                            employee_id=new_employee_id,
            ...
            # STEP 9: Import Payroll (depends on employees) - use ID mapping
            print("Importing payroll...")
            for payroll_data in data.get('payroll', []):
                existing = Payroll.query.filter_by(id=payroll_data['id']).first()
                if not existing:
                    new_employee_id = employee_id_mapping.get(payroll_data['employee_id'])
                    if new_employee_id:
                        payroll = Payroll(
                            id=payroll_data['id'],
                            employee_id=new_employee_id,
            ...
            # STEP 10: Import Time Tracking (depends on employees - LAST) - use ID mapping
            print("Importing time tracking...")
            for entry_data in data.get('time_tracking', []):
                existing = TimeTracking.query.filter_by(id=entry_data['id']).first()
                if not existing:
                    new_employee_id = employee_id_mapping.get(entry_data['employee_id'])
                    if new_employee_id:
                        entry = TimeTracking(
                            id=entry_data['id'],
                            employee_id=new_employee_id,
...
def remove_repurchase_item(item_id):
    ...
    if current_user.user_type not in ['factory', 'office', 'supervisor', 'admin']:
        flash('You do not have permission to modify orders.', 'danger')
        return redirect(url_for('repurchase'))
    
    try:
        # Get the order item
        order_item = RepurchaseOrderItem.query.get_or_404(item_id)
        
        # Check if user owns this order and it's still submitted
        if order_item.order.employee_id != current_user.id:
            flash('You can only remove items from your own orders.', 'danger')
            return redirect(url_for('repurchase'))
        
        if order_item.order.status != 'submitted':
            flash('Only submitted orders can be modified.', 'danger')
            return redirect(url_for('repurchase'))
        ...
        db.session.commit()
        flash(f'Item "{item_name}" removed from order successfully!', 'success')
